...
export function TaskCommandCenterEn({ tasks, onDelete, onUpdate, onCreate }: Props) {
        ...
        <div className="flex items-center justify-between bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-md">
          ...
          <div className="flex items-center gap-6">
            ...
            <span className="text-gray-500 text-sm font-mono uppercase tracking-tighter hidden md:block">
                {filteredTasks.length} units on radar
            </span>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-500 font-mono italic">STATION: {task.id.substring(0,8)}</p>
                  </div>
                </div>
          ...
          <AnimatePresence mode="wait">
            {selectedTask ? (
              ...
              >
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-8">
                    ...
                    <button onClick={() => setSelectedTask(null)} className="text-gray-500 hover:text-white cursor-pointer p-1">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-black/40 p-5 rounded-2xl border border-white/5">
                      <p className="text-gray-500 text-[9px] uppercase font-mono mb-3 tracking-widest flex items-center gap-2">
                        <AlertCircle className="w-3 h-3" /> Description Log
                      </p>
                      <p className="text-gray-300 text-sm leading-relaxed italic font-light">
                        {selectedTask.description || "No additional data in this unit's black box."}
                      </p>
                    </div>
  
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                        <p className="text-gray-500 text-[9px] uppercase font-mono tracking-tighter">Priority</p>
                        <p className={`font-bold text-sm ${selectedTask.priority === 'HIGH' ? 'text-red-500' : 'text-purple-400'}`}>
                          {selectedTask.priority}
                        </p>
                      </div>
                      <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                        <p className="text-gray-500 text-[9px] uppercase font-mono tracking-tighter">Final Window</p>
                ...
                <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/10 mt-6">
                  <button onClick={() => onUpdate(selectedTask)} className="flex items-center justify-center gap-2 bg-white/5 hover:bg-purple-600 text-white p-4 rounded-2xl font-bold text-xs transition-all cursor-pointer group">
                    <Edit3 className="w-4 h-4" /> EDIT
                  </button>
                  <button onClick={() => handleAbort(selectedTask.id)} className="flex items-center justify-center gap-2 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white p-4 rounded-2xl font-bold text-xs transition-all cursor-pointer border border-red-600/20">
              ...
              <div className="w-96 border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center text-gray-600 p-10 text-center">
                <p className="text-[10px] font-mono uppercase tracking-[0.3em] opacity-40">Waiting for target selection...</p>
              </div>
            )}
          </AnimatePresence>
        </div>
