...
export function DashboardEn() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      console.error("Orbit sync failure:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage("Telemetry error: Invalid date format.")
    }
  }
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm group hover:border-purple-500/50 transition-all">
            <div className="flex justify-between items-start">
              <p className="text-gray-400 text-xs font-mono uppercase tracking-tighter">Total in Orbit</p>
          ...
          <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm group hover:border-green-500/50 transition-all">
            <div className="flex justify-between items-start">
              <p className="text-gray-400 text-xs font-mono uppercase tracking-tighter">Missions Accomplished</p>
          ...
          <div className="bg-purple-600/10 border border-purple-500/20 p-8 rounded-3xl backdrop-blur-sm group hover:border-purple-400 transition-all">
            <div className="flex justify-between items-start">
              <p className="text-gray-400 text-xs font-mono uppercase tracking-tighter">Weekly Efficiency</p>
        ...
        {!isAdding && (
          ...
          >
            ...
            <span className="font-bold text-sm tracking-widest uppercase">Add New Mission to Radar</span>
          </button>
        )}
          ...
          <div className="mb-10 bg-white/5 border border-purple-500/30 p-8 rounded-3xl animate-in fade-in slide-in-from-top-4 duration-500">
             ...
             <form onSubmit={handleCreateTask} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                   <label className="text-[10px] font-mono text-gray-500 uppercase">Identification</label>
                   <input 
                    placeholder="Ex: Refactor Propulsion Module"
                ...
                <div className="flex flex-col gap-2">
                   <label className="text-[10px] font-mono text-gray-500 uppercase">Priority Level</label>
                   <select 
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                   <label className="text-[10px] font-mono text-gray-500 uppercase">Details Log</label>
                   <textarea 
                ...
                <div className="flex flex-col gap-2">
                   <label className="text-[10px] font-mono text-gray-500 uppercase">Final Date</label>
                   <input 
                    type="date"
                ...
                <div className="flex items-end gap-3">
                  <button type="submit" className="flex-1 bg-purple-600 hover:bg-purple-500 p-4 rounded-xl font-bold transition-all shadow-lg shadow-purple-600/20 cursor-pointer">INITIATE</button>
                  <button type="button" onClick={() => setIsAdding(false)} className="p-4 rounded-xl text-gray-400 hover:text-white transition-all cursor-pointer">CANCEL</button>
                </div>
             </form>
          </div>
        )}
        ...
        <div className="bg-black/40 border border-white/10 rounded-3xl p-8 backdrop-blur-md">
          ...
          <div className="space-y-4">
            {tasks.length === 0 ? (
              <p className="text-gray-500 text-center py-10 italic">No signals detected on radar...</p>
            ) : (
              tasks.map(task => (
                <div 
                  key={task.id} 
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 font-mono tracking-tighter">{task.description || "No log description"}</p>
                    </div>
                  </div>
